...
def sorting(item_list):
    # Return sorted item list
    # Sort: point =  0.5 * weight + 0.5 * volume + rigidity
    print()


def meat_seafood_bagging(meat_seafood_list, bg:Bagger):
    total_weight = 0
    total_volume = 0
    ...
    volume_bag = int(total_volume / bg.BAG_VOLUME_CAPACITY) + (total_volume %
                                                    bg.BAG_VOLUME_CAPACITY > 0)
    weight_bag = int(total_weight / bg.BAG_WEIGHT_CAPACITY) + (total_weight %
                                                    bg.BAG_WEIGHT_CAPACITY > 0)
    estimate_bag = max(volume_bag, weight_bag)
    print(f"Estimated bags: {estimate_bag}")


def frozen_bagging(frozen_list, bg:Bagger):
...
def food_bagging(food_list, bg:Bagger):
...
def non_food_bagging(non_food_list, bg:Bagger):
...
def display_start_bagging(bg:Bagger):
    ...
    if len(meat_seafood_list):
        meat_seafood_bagging(meat_seafood_list, bg)
    if len(frozen_list):
        frozen_bagging(frozen_list, bg)
    if len(food_list):
        food_bagging(food_list, bg)
    if len(non_food_list):
        non_food_bagging(non_food_list, bg)
